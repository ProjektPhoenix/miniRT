...
/*   Updated: 2025/01/26 12:54:38 by hzimmerm         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
void	process_c(char **array, t_scene *scene, t_parse_flags *check)
{
	...
	check->flag_c = true;
	if (!array[1])
		cleanup_pars_exit(scene, "Error\nCamera is missing features", 2, array);
	...
	if (!array[2])
		cleanup_pars_exit(scene, "Error\nCamera is missing orientation", 2, array);
...
void	process_sp(char **array, t_scene *scene)
{
	...
	if (!array[1])
		cleanup_pars_exit(scene, "Error\nSphere is missing features", 2, array);
	...
	if (!array[2])
		cleanup_pars_exit(scene, "Error\nSphere is missing diameter", 2, array);
	new->diameter = ft_atod(array[2]);
	if (!array[3])
		cleanup_pars_exit(scene, "Error\nSphere is missing color", 2, array);
...
void	process_pl(char **array, t_scene *scene)
{
	...
	if (!array[1])
		cleanup_pars_exit(scene, "Error\nPlane is missing features", 2, array);
	...
	if (!array[3])
		cleanup_pars_exit(scene, "Error\nPlane is missing color", 2, array);
...
void	process_cy(char **array, t_scene *scene)
{
	...
	if (!array[1])
		cleanup_pars_exit(scene, "Error\nCylinder is missing features", 2, array);
	...
	if (!array[3])
		cleanup_pars_exit(scene, "Error\nCylinder is missing diameter", 2, array);
	new->diameter = ft_atod(array[3]);
	if (!array[4])
		cleanup_pars_exit(scene, "Error\nCylinder is missing height", 2, array);
	new->height = ft_atod(array[4]);
	if (!array[5])
		cleanup_pars_exit(scene, "Error\nCylinder is missing color", 2, array);
