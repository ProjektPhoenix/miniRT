...
- checks if other objects block the way to the light source and if not, cals the function to add light contribution */
...
/* calculates and adds the diffuse light based on the angle between the light ray and the object normal vector - calls the specular light function and adds this on top */
...
static bool check_cyls_blocking(t_ray *l_ray, t_scene *scene)
{
	t_cylinder	*temp_c;
	double		t;
