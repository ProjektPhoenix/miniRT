...
double	find_t_cylinder(t_ray *ray, t_cylinder *cyl)
{
	...
	// debug("Ray direction unit vector: (%f,%f,%f)", ray->dir.e[0], ray->dir.e[1], ray->dir.e[2]);
	...
	c.cross_ray_cyl = scalar_mply_vector(-1.0, cross_product(ray->dir, c.cyl_dir_unit));
	// debug("Cross product ray and cyl direction: (%f,%f,%f)", c.cross_ray_cyl.e[0], c.cross_ray_cyl.e[1], c.cross_ray_cyl.e[2]);
	// debug("Value of discrimant components:\ndot product of cross ray/cyl with itself: %f\nRadius: %f\nDot product base and cross ray/cyl vector: %f", dot_product(c.cross_ray_cyl, c.cross_ray_cyl), c.radius, dot_product(c.orig_to_base, c.cross_ray_cyl));
	...
	if (c.discriminant >= 0 && get_magnitude(c.cross_ray_cyl) != 0)
	{
		...
		// if ((temp >= 0 && dist_from_cyl_base(&c, temp) >= -2.0 && dist_from_cyl_base(&c, temp) < (cyl->height + 2)) || (temp2 >= 0 && dist_from_cyl_base(&c, temp2) >= -2.0 && dist_from_cyl_base(&c, temp2) < (cyl->height + 2)))
		// {
		// 	debug("temp cyl: %f, temp 2 cyl: %f", temp, temp2);
	...
	if (dot_product(c.cyl_dir_unit, ray->dir) != 0)
	{
		...
		// if (temp >= 0 || temp2 >= 0)
		// 	debug("temp cyl lid: %f, temp2 cyl lid: %f", temp, temp2);
		...
		{
			t = distance;
			// debug("Distance is > 0 and therefore t cylinder set to: %f", t);
		}
	}
